...
# 주식 일봉 차트 조회 함수 (ka10081)
def get_daily_chart(base_url, token, stock_code):
    """키움 Open API를 통해 주식 일봉 차트 데이터를 조회합니다 (ka10081)."""
    host = base_url
    ...
    data = {
        "stk_cd": stock_code,
        "chart_dv": "D",  # D: 일봉, W: 주봉, M: 월봉
        "s_dt": (datetime.datetime.now() - datetime.timedelta(days=365)).strftime('%Y%m%d'), # 시작일 (1년 전)
        "e_dt": datetime.datetime.now().strftime('%Y%m%d'), # 종료일 (오늘)
        "period_dv": "D" # D: 일, W: 주, M: 월
    }
    try:
        response = requests.post(url, headers=headers, json=data)
        response.raise_for_status()
        res_json = response.json()
        logger.info(f'\n--- 주식 일봉 차트 조회 응답 (ka10081) for {stock_code} ---')
        logger.info(f"Code: {response.status_code}")
        logger.info(f"Body: {json.dumps(res_json, indent=4, ensure_ascii=False)}")
        return res_json.get('data')
    except requests.exceptions.RequestException as e:
        logger.error(f"주식 일봉 차트 조회 요청 (ka10081) 중 오류 발생: {e}")
        return None
    except json.JSONDecodeError:
        logger.error(f"주식 일봉 차트 조회 응답 (ka10081) JSON 파싱 오류. 응답: {response.text}")
        return None

# 주식 분봉 차트 조회 함수 (ka10080)
def get_minute_chart(base_url, token, stock_code):
    """키움 Open API를 통해 주식 분봉 차트 데이터를 조회합니다 (ka10080)."""
    host = base_url
    endpoint = '/api/dostk/chart'
    url = host + endpoint
    ...
    data = {
        "stk_cd": stock_code,
        "chart_dv": "M",  # M: 분봉
        "s_dt": datetime.datetime.now().strftime('%Y%m%d'), # 시작일 (오늘)
        "e_dt": datetime.datetime.now().strftime('%Y%m%d'), # 종료일 (오늘)
        "period_dv": "1" # 1, 5, 10, 30, 60 분
    }
    try:
        response = requests.post(url, headers=headers, json=data)
        response.raise_for_status()
        res_json = response.json()
        logger.info(f'\n--- 주식 분봉 차트 조회 응답 (ka10080) for {stock_code} ---')
        logger.info(f"Code: {response.status_code}")
        logger.info(f"Body: {json.dumps(res_json, indent=4, ensure_ascii=False)}")
        return res_json.get('data')
    except requests.exceptions.RequestException as e:
        logger.error(f"주식 분봉 차트 조회 요청 (ka10080) 중 오류 발생: {e}")
        return None
    except json.JSONDecodeError:
        logger.error(f"주식 분봉 차트 조회 응답 (ka10080) JSON 파싱 오류. 응답: {response.text}")
        return None
...
if __name__ == "__main__":
    ...
    if token:
        ...
        if db_info:
            db_token = db_info[0]
            logger.info(f"토큰 (DB에서): {db_token[:5]}**********")
            logger.info(f"계좌 번호 (DB에서): {db_info[1]}")
            logger.info(f"계좌명 (DB에서): {db_info[2]}")
            logger.info(f"잔액 (DB에서): {db_info[3]}")

            # --- 차트 데이터 조회 테스트 (삼성전자: 005930) ---
            stock_code_to_test = "005930"
            
            # 일봉 데이터 조회
            daily_data = get_daily_chart(base_url, db_token, stock_code_to_test)
            if daily_data:
                logger.info(f"성공: {stock_code_to_test} 일봉 데이터 일부를 가져왔습니다.")
                # logger.info(json.dumps(daily_data[:3], indent=4, ensure_ascii=False)) # 너무 길어서 일부만
            else:
                logger.error(f"실패: {stock_code_to_test} 일봉 데이터를 가져오지 못했습니다.")

            # 분봉 데이터 조회
            minute_data = get_minute_chart(base_url, db_token, stock_code_to_test)
            if minute_data:
                logger.info(f"성공: {stock_code_to_test} 분봉 데이터 일부를 가져왔습니다.")
                # logger.info(json.dumps(minute_data[:3], indent=4, ensure_ascii=False)) # 너무 길어서 일부만
            else:
                logger.error(f"실패: {stock_code_to_test} 분봉 데이터를 가져오지 못했습니다.")
